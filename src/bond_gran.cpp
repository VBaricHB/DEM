...
void BondGran::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    dely = x[i1][1] - x[i2][1];
    delz = x[i1][2] - x[i2][2];
    // Checks if the particles are across the border
    domain->minimum_image(delx,dely,delz);
    // ATTENTION: 11.08.16 Changed by Valentin Baric, to account for the fact
    //            that the bond does not start at the origin of the particles but
    //            where the surface of the bond touches the surface of the particle
    rsq = delx*delx + dely*dely + delz*delz;
    rsqinv = 1./rsq;
    r = sqrt(rsq);
    rinv = 1./r;
    // r = sqrt(delx*delx + dely*dely + delz*delz) - sqrt(radius[i1]*radius[i1]-rbmin*rmin) - sqrt(radius[i2]*radius[i2]-rbmin*rmin);
    // rinv = 1./r;
    // rsq = r*r;
    // rsqinv = 1./rsq;
    ...
    else //NP stress or stress_temp
    {
        ...
        {
            bondlist[n][3] = 1;
            fprintf(screen,"broken bond between %d and id %d at step %d",bondlist[n][0],bondlist[n][1],update->ntimestep);
            if(toohot)fprintf(screen,"  (too hot)");
            if(nstress)fprintf(screen," (nstress)");
            if(tstress)fprintf(screen," (tstress)");
	    fprintf(screen,"\n");
        }
    }

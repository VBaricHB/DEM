...
void FixBondCreateGran::post_integrate()
{
  ...
  double xtmp,ytmp,ztmp,delx,dely,delz,rsq,min,max,r;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      dely = ytmp - x[j][1];
      delz = ztmp - x[j][2];
      // ATTENTION: 11.08.16 This is changed by Valentin Baric
      // To account for the fact, that the bond does not start at the center of mass
      // but at the point where the skin of the bond touches the particle
      ...
      //Modified by V.B. (08.08.16) to set bonds only within molecules
      if (atom->molecule[i] != atom->molecule[j]){
	   fprintf(screen,"skipping bond, different molecules for %d and %d\n",i,j);     
	   continue; 
      }
